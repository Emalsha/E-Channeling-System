DROP TYPE ACMA_DOC_HISTORY_TBL

--OBJ
CREATE OR REPLACE TYPE ACMA_DOC_HISTORY_OBJ AS OBJECT
...
--TBL
CREATE OR REPLACE TYPE ACMA_DOC_HISTORY_TBL IS TABLE OF ACMA_DOC_HISTORY_OBJ;

--Function
CREATE OR REPLACE FUNCTION ACMA_DOCTOR_APPOINMENT_HISTORY(doctor_id_ NUMBER) 
RETURN ACMA_DOC_HISTORY_TBL
AS RES_TBL ACMA_DOC_HISTORY_TBL;
BEGIN

  SELECT ACMA_DOC_HISTORY_OBJ(ap.appoinment_id, ap.patient_id, p.fullname, ap.created_date_time, ap.appointment_time, ap.consulting_catogery, ap.status) 
  BULK COLLECT INTO RES_TBL 
  FROM acma_appoinment ap
  LEFT JOIN acma_patient p on ap.patient_id = p.patient_id
  WHERE AP.DOCTOR_ID = DOCTOR_ID_ order by ap.appoinment_id;
  RETURN RES_TBL;
END;
...
-- Testing
declare
  datas ACMA_DOC_HISTORY_TBL;
begin
  datas := ACMA_DOCTOR_APPOINMENT_HISTORY(41);
