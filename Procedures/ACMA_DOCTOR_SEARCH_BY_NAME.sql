...
BEGIN
  ...
  FROM ACMA_DOCTOR D
  JOIN ACMA_DOCTOR_SPECIALTY DS ON D.DOCTOR_ID = DS.DOCTOR_ID
  JOIN ACMA_SPECIALTY S ON DS.SPECIALTY_ID = S.SPECIALTY_ID
...
------------------------------------ SQL
SELECT D.DOCTOR_ID,D.FULLNAME,D.AVAILABLE_ON_WEEKEND,D.ROOM_NUMBER,S.DESCRIPTION
FROM ACMA_DOCTOR D
JOIN ACMA_DOCTOR_SPECIALTY DS ON D.DOCTOR_ID = DS.DOCTOR_ID
JOIN ACMA_SPECIALTY S ON DS.SPECIALTY_ID = S.SPECIALTY_ID
WHERE lower(FULLNAME) LIKE lower('%cha%')
ORDER BY D.FULLNAME 
...
BEGIN
  RES_LIST := ACMA_DOCTOR_SEARCH_BY_NAME('san');
